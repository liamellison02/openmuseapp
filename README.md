# OpenMuse NBA RAG Chatbot

A fully-featured Retrieval-Augmented Generation (RAG) chatbot for answering basketball questions. The project couples **Next.js (14 / App Router)** for the frontend & API layer with **LangChain**, **OpenAI-compatible LLMs**, and **MongoDB Atlas Vector Search** for contextual retrieval.
...
- **Serverless-first**: all compute is in Vercel edge / serverless functions; MongoDB Atlas handles persistence
...
└─────┬────────┘                                        └──────────┬──────────────┘
      ...
      │              │                                            ▼
      │              │                                ┌──────────────────────────┐
      │              │        4. formatted answer     │  LLM Provider (ai-sdk)  │
      │              └────────────────────────────────│  – GPT-4o-mini          │
      │                                               └──────────┬──────────────┘
...
| **Embeddings** | `text-embedding-ada-002` via LangChain `OpenAIEmbeddings` |
| **LLM** | Default: `openrouter:openai/gpt-4o-mini`; can be swapped via `lib/ai-config.ts` |
...
├─ next.config.mjs      # Vercel toolbar & Next options
├─ package.json
